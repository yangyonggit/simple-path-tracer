...
glm::vec3 PathTracer::m_material_manager.getColorFromGeometryID(int geomID) {
    switch(geomID) {
...
glm::vec3 PathTracer::m_material_manager.getMaterialAlbedo(int geomID) {
    switch(geomID) {
        case 0: return glm::vec3(1.0f, 0.71f, 0.29f); // Gold sphere
        case 1: return glm::vec3(0.95f, 0.93f, 0.88f); // Silver sphere
        case 2: return glm::vec3(0.95f, 0.64f, 0.54f); // Copper sphere
...
glm::vec3 PathTracer::tracePathMonteCarlo(RTCScene scene, const glm::vec3& origin, 
    ...
    if (rayhit.hit.geomID == RTC_INVALID_GEOMETRY_ID) {
        return m_environment_manager.getCubemapColor(glm::normalize(direction));
    }
...
glm::vec3 PathTracer::m_environment_manager.getSkyColor(const glm::vec3& direction) {
...
glm::vec3 PathTracer::m_environment_manager.getCubemapColor(const glm::vec3& direction) const {
    if (m_cubemap.isLoaded()) {
        ...
        return m_environment_manager.getSkyColor(direction);
    }
}

// ACES Filmic tone mapping implementation
glm::vec3 PathTracer::m_environment_manager.acesToneMapping(const glm::vec3& color) {
    // ACES constants
    const float a = 2.51f;
    const float b = 0.03f;
    const float c = 2.43f;
